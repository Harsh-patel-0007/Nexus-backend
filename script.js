...
async function loadFoldersFromDB() {
    try {
        ...
        folders.forEach(folder => {
            ...
            console.log(folder)
            createFolderCard(folder.folderName,folder.userId, folder._id, folder.date);
        });
...
function createFolderCard(name,userId, folderId, dateStr) {
    ...
    // Click → open file preview
    newItem.addEventListener('click', () => {
        openFolder(folderId,userId, name);
    });
...
function openFolder(folderId,userId, name) { document.getElementById('folderoverlay').classList.add('active'); }
// function openFolder(folderId, name) { document.getElementById('folderoverlay.foldercont').classList.add('active'); }

function closeFolder(folderId,userId, name) { document.getElementById('folderoverlay').classList.remove('active'); }
...
async function openFolder(folderId, name) {
    ...
    try {
        ...
        const res = await fetch(`http://127.0.0.1:5000/api/files/${folderId}`,{
            credentials: "include"
        })
        // credentials: true
        const files = await res.json();
...
async function uploadToFolder() {
    ...
    try {
        ...
        const res = await fetch(`http://127.0.0.1:5000/api/upload/${activeFolderId}`, {
            method: 'POST',
            body: formData,
            credentials: "include"
        });
...
async function signin() {
    ...
    try {
        ...
        if (res.status === 200) {
            ...
            window.location.href = 'http://127.0.0.1:5500/frontend/index.html'            // 👈 change to your page
...
// //for the community files 
// async function uploadCommunityFile() {
//     const fileInput = document.getElementById('communityfileInput');   // the file picker
//     const fileNameInput = document.getElementById('communityimageName'); // the text input
...
//     const formData = new FormData();
//     formData.append('file', file);          // key: 'file'     → the binary file
//     formData.append('fileName', fileName);  // key: 
//     formData.append('filetype', file.type); // e.g. "image/
...
//     try {
//         const response = await fetch('http://localhost:5000/api/community-uploads', {
//             method: 'POST',
//             body: formData,
//             credentials: "include"
//         });
